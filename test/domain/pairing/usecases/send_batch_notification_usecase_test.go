...
func TestSendBatchNotificationUseCase_Execute(t *testing.T) {
	tests := []struct {
		...
		setupMocks    func(*mocks.MockPortNotificationWriter, *mocks.MockPortUserNotificationPreferencesReader, *mocks.MockNotificationSender)
	...
	}{
		{
			...
			setupMocks: func(writer *mocks.MockPortNotificationWriter, prefsReader *mocks.MockPortUserNotificationPreferencesReader, sender *mocks.MockNotificationSender) {
				prefsReader.On("GetByUserID", mock.Anything, mock.Anything).Return(func(ctx context.Context, userID uuid.UUID) *pairing_entities.UserNotificationPreferences {
		...
		},
		{
			...
			setupMocks: func(writer *mocks.MockPortNotificationWriter, prefsReader *mocks.MockPortUserNotificationPreferencesReader, sender *mocks.MockNotificationSender) {
				sender.On("GetChannel").Return(pairing_entities.NotificationChannelSMS)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tt.setupMocks(writer, prefsReader, sender)
